    ...
    public function assemblyOrReturn() {
        ...
        if (isset($_POST['assembly'])) {
                ...
                foreach ($partIds as $partId) {
                    ...
                    if (($key = array_search($pieceId, $pieceIds)) !== FALSE) {
                        if ($pieceId == 1) {
                            ...
                            $model[0] = $this->parts->get($partId)->model;
                        } else if ($pieceId == 2) {
                            $orderedPartsIds[1] = $partId;
                            $model[1] = $this->parts->get($partId)->model;
                        } else {
                            $orderedPartsIds[2] = $partId;
                            $model[2] = $this->parts->get($partId)->model;
                        }
                        unset($pieceIds[$key]);
                    } else {
            ...
            if (count($error_msgs) == 0) {
                ...
                $robot->status = 1;
                $robot->part1model = $model[0];
                $robot->part2model = $model[1];
                $robot->part3model = $model[2];
                $this->robots->add($robot);

                foreach ($orderedPartsIds as $orderedPartsId) {
                    $this->parts->delete($orderedPartsId);
                }
